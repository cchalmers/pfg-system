...
import Control.Lens                       ((^.))
import Data.ByteString.Builder           as Blaze
...
import Diagrams.TwoD.Types      (V2, P2, unr2, unp2, r2)
...
renderWith :: Surface -> Bool -> V2 Double -> Put -> Builder
...
show4 :: Double -> Put
show4 x = rawString $ showFFloat (Just 4) x ""
-- show4 = raw . toFixed 4
...
moveTo :: V2 Double -> Put
moveTo v = ln $ do
  sys "moveto"
  p v

lineTo :: V2 Double -> Put
lineTo v = ln $ do
  sys "lineto"
  p v

curveTo :: V2 Double -> V2 Double -> V2 Double -> Put
...
clip :: Clip Double -> Put
clip (Clip paths) = mapM_ clipPath paths

clipPath :: Path V2 Double -> Put
...
path :: Path V2 Double -> Put
path (Path trs) = mapM_ locTrail trs

-- locTrail :: Located (Trail V2 Double) -> Put
-- locTrail (viewLoc -> (unp2 -> p2, t)) = do
--   moveTo (r2 p2)
--   trail t
-- 
-- trail :: Trail V2 Double -> Put
...
moveSeg :: Segment Closed V2 Double -> P2 Double -> Put
...
segment :: Segment Closed V2 Double -> Put
segment (Linear (OffsetClosed v))       = lineTo v
segment (Cubic v1 v2 (OffsetClosed v3)) = curveTo v1 v2 v3

hbox :: Hbox Double -> Put
hbox (Hbox tr str) = do
  shift $ T.transl tr
  pgfHbox $ rawString str

defObject :: Put -> V2 Double -> V2 Double -> Put -> Put
defObject name l u r = do
  sys "defobject"
  bracers name
  bracers (p l)
  bracers (p u)
...
useObject :: String -> Put -> Put
useObject name extras = do
  sys "useobject"
  bracers name
  lnBracers extras

rect :: P2 Double -> V2 Double -> Put
rect (P (V2 x y)) (V2 w h) = do
  sys "rect"
  bracers $ n x
  bracers $ n y
  bracers $ n w
